...
int cnt = 0;
int sstime = 500; // sstime : 센서측정간격
int sitTime = 0; // 앉은시간측정, 자세가 바르던 나쁘던 앉기만하면됨(FSR에 뭔가 감지되기만 하면)
int stretchTime = 5000; // 이 값 이상으로 앉은시간(sitTime) 측정되면 스트레칭용 알림전송
unsigned long sitStretch = 0;
int ledDuration = 500; // led 점멸 시간 간격
...
int but_onoff = 5, pres_onoff = 0, cnt_onoff = 0; // 실시간 알림기능 onoff용 버튼, 초기상태는 on
int but_capture = 4; // 버튼 누를 때 lcd에 실시간 자세 알려주는 버튼

int redPin = 9; // 실시간 알림용 led
int bluePin = 10; 
int greenPin = 11;

// 바른자세,엉덩이 멀어진자세, 좌로 치우침, 우로 치우침, 허리만 굽음 순서임
int posture[5] = {0,0,0,0,0,0};
String pos[5] = {"  Good Posture! ", " Far from Back! ", "   Bend Left!   ", "   Bend Right!  ", "  Back Curved!  "}; 
int reportTime = 50000; //50초마다 레포트전송
